...
class Diffusion:
    def __init__(self, T=500, beta_start=1e-4, beta_end=0.02, img_size=256, scheduler = 'linear', device="cuda"):
        """
    ...
    def get_betas(self):
        if self.scheduler == 'linear':
            ...
            s = 0.008
            f_t = [torch.cos(torch.tensor((((t/self.T)+s)/(1+s))*torch.pi/2))**2 for t in range(0,self.T+1)] 
            alpha = [f_t[i]/f_t[0] for i in range(0,self.T+1)]
            beta_t = [1-alpha[i]/alpha[i-1] for i in range(1,self.T+1)]
            beta = torch.clamp(torch.tensor(beta_t), max=0.999)
            return beta
    ...
    def p_sample(self, model, x_t, t):
        ...
        if t[0] > 1: # note t is a tensor of size batchsize
            noise = torch.normal(torch.zeros(self.img_size), torch.eye(self.img_size))
        else:
            noise =  torch.zeros(self.img_size)
...
if __name__ =='__main__':
    from new_bratsloader import load_file
    import os
    import matplotlib.pyplot as plt

    img, _ = load_file(os.path.expanduser("~/Desktop/ADLCV-AnomalyDetection/data/brats/val/BraTS20_Training_295/brats_train_295_100.nii.gz"))
    # device = 'cuda' if torch.cuda.is_available() else 'cpu'
    device = 'cpu'
    scheduler = 'cosine'
    diff = Diffusion(T = 1000, img_size=img.size(1), scheduler = scheduler, device = device)
    timestep = torch.tensor([999,])
    noised_img, noise = diff.q_sample(img[0,:,:], timestep)
    plt.imshow(noised_img[0, 0, :, :].numpy())
    plt.colorbar()
    plt.savefig(F'diffusion{scheduler}{timestep.item()}.png')
